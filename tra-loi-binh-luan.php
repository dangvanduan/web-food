    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-9">
                                    ...
                                    <form method="GET" action="cart.php">
                                        ...
                                        <div class="quantity clearfix">
                                            ...
                                            <div class="quantity-input">
                                                <input type="number" value="1" class="text" name="quantity" ng-model="InputQuantity" ng-init="InputQuantity=1" />
                                            </div>
                                        ...
                                        <div class="button">
                                            <input type="submit" value="Thêm giỏ hàng"></input>
                                            <a href="thanh-toan.php" class="btn btn-primary"> Mua ngay</a>
                                        </div>
                                        </form>
                ...
                $select12 = "SELECT users.user_id,users.full_name,comment.product_id,comment.comment_id,comment.comment_id,comment.content,comment.created_at FROM users INNER JOIN comment ON users.user_id=comment.user_id WHERE comment_id={$comment_id}";
                ...
                ?>                         
                    ...
                    if(isset($_POST['submit'])){
                        ...
                        if(empty($content)) {
                            echo "<script>alert('Vui lòng nhập nội dung vào khung chat!')</script>";
                        }else{
                            $query11 = "INSERT INTO comment(user_id,product_id,content,parent_id) VALUE($user_id,$product_id,'$content',$comment_id)";
              ...
              <?php
                ...
                ?>
                    <div class="alert alert-danger">
                        ...
                        <strong>Vui lòng đăng nhập để bình luận! </strong><a href="dang-nhap.php"> Login </a>
                    </div>
                <?php
                }
            }
                ?>
                <!-- comment con  -->
                <?php $cmt=$_GET['comment_id'];
                $sql="SELECT users.user_id,users.full_name,comment.product_id,comment.parent_id,comment.comment_id,comment.comment_id,comment.content,comment.created_at FROM users INNER JOIN comment ON users.user_id=comment.user_id WHERE parent_id=$cmt";
                $re=$mysqli->query($sql);
                
                while($arCon=mysqli_fetch_assoc($re)){
                    if($arCon['parent_id']!=0){
                ?>
                <div class="commentCon">
                <div class="form-groupCon">
                <span><img src="Uploads/shop97/images/avatar.jpg" alt="" width=50px height=50px;><?php echo " ".$arCon['full_name'] ?></span>              
                    <h5>Ngày đăng: <?php echo $arCon['created_at']?></h5>
                    <p><?php echo $arCon['content']?></p>
                    <hr>
                </div>
                <?php
                }
